...
pub async fn create_city_tables(pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let sql = r#"SET client_min_messages TO WARNING; 
            ...
            drop table if exists geo.city_names;
            ...
            create index city_names_city_id on geo.city_names(city_id);
            create index city_names_alt_name on geo.city_names(alt_name);
            ...
            create index city_name_comp_name on mdr.city_names(comp_name);
...
pub async fn import_data(data_folder: &PathBuf, source_file_name: &str, pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    make_mdr_related_changes_2(pool).await?;
    make_mdr_related_changes_3(pool).await
...
async fn transfer_cities_data(pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let sql = r#"insert into geo.cities(id, name, disamb_type, disamb_id, disamb_code, disamb_name, 
...
async fn remove_dup_cities_1(pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let sql = r#"SET client_min_messages TO WARNING; 
        drop table if exists geo.temp_city_dups;
        create table geo.temp_city_dups as
        select  
        ...
        drop table if exists geo.temp_city_dup_recs;
        create table geo.temp_city_dup_recs as
        select  
            g.*, true as to_drop
            from geo.cities g
            inner join geo.temp_city_dups d
            on g.name = d.name
            and g.disamb_name = d.disamb_name
            and g.country_name = d.country_name;

        drop table if exists geo.temp_max_pop_dups;
        create table geo.temp_max_pop_dups as 
        select name, disamb_name, country_name, max(pop) as maxpop
        from geo.temp_city_dup_recs
        group by name, disamb_name, country_name;

        update geo.temp_city_dup_recs d
        set to_drop = false
        from geo.temp_max_pop_dups m
        where d.name = m.name
    ...
    let sql = r#"delete from geo.cities c
        using geo.temp_city_dup_recs d
        where c.id = d.id
        and d.to_drop = true;"#;
    ...
    let sql = r#"SET client_min_messages TO WARNING; 
        drop table if exists temp_city_dup_recs;
        drop table if exists temp_max_pop_dups;
        drop table if exists temp_city_dups;"#;
...
async fn remove_dup_cities_2(pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let sql = r#"SET client_min_messages TO WARNING; 
            ...
            drop table if exists geo.temp_min_id_dups;
            create table geo.temp_min_id_dups as 
    ...
    let sql = r#"SET client_min_messages TO WARNING; 
            drop table if exists temp_city_dup_recs;
            drop table if exists temp_min_id_dups;
            drop table if exists temp_city_dups;"#;
...
async fn create_city_names(pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let sql = r#"insert into geo.city_names (city_id, city_name, disamb_id, 
...
async fn transfer_names_to_mdr(pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let sql = r#"insert into mdr.city_names (comp_name, city_id, city_name, disamb_id, 
...
async fn make_mdr_related_changes_1(pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let sql = r#"delete from mdr.city_names where comp_name = 'chicago' and disamb_name = 'Ohio';
    delete from mdr.city_names where comp_name = 'new york' and disamb_name = 'Nebraska';"#;
    ...
    let params_set: Vec<Vec<&str>> = vec![
        ...
        let sql = r#"insert into mdr.city_names(comp_name, city_id, city_name, disamb_id, disamb_name, country_id, country_name, source)
                    ...
                    insert into mdr.city_names(comp_name, city_id, city_name, disamb_id, disamb_name, country_id, country_name, source)
...
async fn make_mdr_related_changes_2(pool: &Pool<Postgres>) -> Result<(), AppError> {
...
async fn make_mdr_related_changes_3(pool: &Pool<Postgres>) -> Result<(), AppError> {
...
async fn add_city_recs(params: Vec<Vec<&str>>, pool: &Pool<Postgres>) -> Result<(), AppError> {

    let mut sql = "".to_string();
    for p in params {
         
         let this_sql = r#"insert into mdr.city_names(comp_name, city_id, city_name, disamb_id, disamb_name, country_id, country_name, source)
                "#.to_string()
